...
> **Read [MAP.md](MAP.md) before opening `index.html`.** It is 14,500 lines and
> nobody needs all of it. The map is generated by `tools/map.py`: 79 sections,
> what each one is for, and for every one of them the names other sections call
> (`surface`) and who calls them (`used by`). Jump with
> `python tools/map.py show <id> -b`, ask `python tools/map.py where <name>`
> who owns a name, and run `python tools/map.py` after any edit that moves
> lines — `--check` tells you whether it has gone stale.
